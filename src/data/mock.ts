...
const namedUserKeys: SshKey[] = [
  ...
  { id: "uk12", name: "FetchKey_shared_deploy",   type: "user", encryption: "RSA",     length: 2048, age: "180 days", associatedUsers: ["user1", "user2", "user3"], clientEndpoints: ["10.0.3.5"],     hostEndpoints: ["10.0.3.6"],      fingerprint: fp("gH6aNt2pOvUwX0yZ5b"),   status: "Active", riskStatus: "Shared",     complianceStatus: "Non-Compliant", filePaths: ["/home/shared/.ssh/"],            comment: "Shared deployment key - TO REPLACE", keyComplianceGroup: "Default",   hasCert: false, certCount: 0, combination: "private_public"  },
];
...
export interface Host {
  id: string; deviceName: string; fqdn: string; hostName: string;
  hostNameStatus: "active" | "inactive"; group: string;
  hostStatus: "Managed" | "Unmanaged"; client: string; accessType: string;
  username: string; port: number; vendor: string; lastSyncTime: string;
  instanceId: string; hostPermission: string; allowedPrincipals: string;
}
...
export const USER_CERTS: SshCert[] = [
  ...
  { id: "uc7", certKeyId: "UC-2026-007", certName: "cert-orphaned-svcbatch",     type: "user", associatedKeyId: "unmanaged-external-key", associatedKeyName: "external-unmanaged-key", principals: ["svc-batch"],         caName: "Default-Infra-CA", serialNumber: "901234567890", status: "Active",  validFrom: "2026-04-01", validTo: "2026-07-01", expiresIn: "41 days",    expiresInDays: 41,  endpoints: ["10.0.5.30"],                extensions: ["pty"],                                                      complianceStatus: "Non-Compliant" },
...
export interface RotatedCert {
  id: string; certKeyId: string; certType: "User" | "Host";
  rotatedOn: string; previousValidTo: string; newValidTo: string;
  rotatedBy: string; endpoints: string[]; rollbackAvailable: boolean;
  rollbackWindowExpiry: string; associatedKeyId: string;
}
...
export interface DeletedCert {
  id: string; certKeyId: string; certType: "User" | "Host";
  deletedOn: string; deletedBy: string; reason: string;
  lastKnownEndpoints: string[]; isRevoked: boolean; isExpired: boolean;
  validTo: string; associatedKeyId: string; associatedKeyName: string;
}
...
export interface DeletedKey {
  id: string; name: string; encryption: string; length: number;
  deletedOn: string; deletedBy: string; endpoints: string[];
}
...
export interface RotatedKey {
  id: string; name: string; encryption: string; length: number;
  rotatedOn: string; rotatedBy: string; endpoints: string[];
}
