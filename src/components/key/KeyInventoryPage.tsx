      ...
      <div className="flex items-center gap-2 bg-surface border border-border rounded-t-md px-3 py-2">
        ...
        <div className="ml-auto flex items-center gap-1">
          <Button
            size="sm"
            variant="ghost"
            onClick={() => toast.info("Add Key — Coming Soon")}
          >
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56">
              <DropdownMenuItem onClick={() => setProvisionFor(selectedKeys[0])}>
                Provision Key &amp; Certificate
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => toast.success("Key updated.")}>
                Modify
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => {
              ...
              <DropdownMenuItem onClick={() => toast.success(`Exported ${selectedKeys.length} key${selectedKeys.length > 1 ? "s" : ""} to CSV.`)}>
                Export
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => toast.success("Key file download started.")}>
                Download
              </DropdownMenuItem>
              <DropdownMenuItem
                disabled={!(selectedKeys[0]?.status === "Active" && selectedKeys[0]?.hasCert)}
                onClick={() => setConfirmRevoke(selectedKeys[0])}
              >
                Revoke
              </DropdownMenuItem>
              ...
              <DropdownMenuItem
                disabled
                onClick={() => toast.info("No rollback available for this key.")}
              >
                Rollback
              </DropdownMenuItem>
              <DropdownMenuSub>
              ...
              <DropdownMenuItem onClick={() => toast.success("Key group updated.")}>
                Change Key Group
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
      ...
      <div className="bg-surface border border-t-0 border-border rounded-b-md overflow-x-auto">
        <table className="w-full text-[13px]">
          ...
          <tbody>
                ...
                <Row key={k.id}>
                  ...
                  {isExpanded && (
                    <tr className="border-t border-border bg-muted/30">
                      <td colSpan={visibleCols.length + 2} className="px-12 py-3 text-[12px]">
            ...
            {paged.length === 0 && (
              <tr>
                <td colSpan={visibleCols.length + 2} className="px-3 py-12 text-center text-muted-foreground">
        ...
        description={
          <div className="space-y-2 pt-2">
            <label className="flex items-center gap-2 text-[13px]">
              <input
                type="radio"
                checked={statusNew === "Active"}
                onChange={() => setStatusNew("Active")}
              />
              Active
            </label>
            <label className="flex items-center gap-2 text-[13px]">
              <input
                type="radio"
                checked={statusNew === "Inactive"}
                onChange={() => setStatusNew("Inactive")}
              />
              Inactive
            </label>
          </div>
...
function renderCell(k: SshKey, col: string, opts: { onCertClick: () => void }): React.ReactNode {
  switch (col) {
    ...
    case "associatedUsers":
      return (
        <Truncated items={k.associatedUsers} />
      );
...
function Truncated({ items, mono }: { items: string[]; mono?: boolean }) {
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      {items.length > 1 && (
        <Badge variant="outline" className="h-5 text-[10px]">+{items.length - 1}</Badge>
      )}
    </div>
  );
}
