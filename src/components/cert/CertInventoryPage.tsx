...
export function CertInventoryPage({
  ...
  const getSelectedCombination = (cert: SshCert | undefined): CertCombination => {
    if (!cert) return "cert_only";
    const keyPool = cert.type === "user" ? USER_KEYS : HOST_KEYS;
    const key = keyPool.find((k) => k.id === cert.associatedKeyId);
    return key?.combination ?? "cert_only";
  };
  const selCombination = getSelectedCombination(selected[0]);
  ...
  return (
    <div>
      ...
      <FilterChips
        chips={chips}
        onRemove={() => { setTileFilter(null); navigate({ to: ".", search: {} as never }); }}
      />
        ...
        <div className="ml-auto flex items-center gap-1">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              {/* Revoke -- status gated */}
              {(() => {
                const a = selected[0] ? certRevokeAction(selected[0].status) : { show: false, enabled: false };
                if (!a.show) return null;
                if (!a.enabled)
                  return (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <DropdownMenuItem disabled><span className="pointer-events-auto">Revoke</span></DropdownMenuItem>
                      </TooltipTrigger>
                      <TooltipContent side="right" className="max-w-xs text-xs">{a.tooltip}</TooltipContent>
                    </Tooltip>
                  );
                return <DropdownMenuItem onClick={() => setRevokeCert(selected[0])}>Revoke</DropdownMenuItem>;
              })()}

              {/* Rotate Certificate -- combination + status gated */}
              {(() => {
                const a = selected[0] ? certRotateAction(selected[0].status, selCombination) : { show: false, enabled: false };
                if (!a.show) return null;
                if (!a.enabled)
                  return (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <DropdownMenuItem disabled><span className="pointer-events-auto">Rotate Certificate</span></DropdownMenuItem>
                      </TooltipTrigger>
                      <TooltipContent side="right" className="max-w-xs text-xs">{a.tooltip}</TooltipContent>
                    </Tooltip>
                  );
                return <DropdownMenuItem onClick={() => setRotateCert(selected[0])}>Rotate Certificate</DropdownMenuItem>;
              })()}

              <DropdownMenuItem onClick={() => toast.success("Exported cert data to CSV.")}>Export</DropdownMenuItem>
              <DropdownMenuItem onClick={() => toast.success("Certificate file download started.")}>Download Certificate</DropdownMenuItem>
              <DropdownMenuItem onClick={() => toast.success("Status updated.")}>Change Status</DropdownMenuItem>

              {/* Delete -- status gated */}
              {(() => {
                const a = selected[0] ? certDeleteAction(selected[0].status) : { show: false, enabled: false };
                if (!a.show) return null;
                if (!a.enabled)
                  return (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <DropdownMenuItem disabled className="text-risk-red/50"><span className="pointer-events-auto">Delete Certificate</span></DropdownMenuItem>
                      </TooltipTrigger>
                      <TooltipContent side="right" className="max-w-xs text-xs">{a.tooltip}</TooltipContent>
                    </Tooltip>
                  );
                return (
                  <DropdownMenuItem className="text-risk-red focus:text-risk-red focus:bg-risk-red/10" onClick={() => setDeleteCert(selected[0])}>
                    Delete Certificate
                  </DropdownMenuItem>
                );
              })()}
          ...
          <table className="inv-table">
            ...
            <tbody>
              {filtered.map((c) => {
                ...
                const validToCls = c.expiresInDays <= 0 ? "text-risk-red" : c.expiresInDays <= 30 ? "text-risk-amber" : "";
                return (
                  <tr key={c.id} onClick={() => setDrawerCert(c)} className={cn("cursor-pointer", isSel && "row-selected")}>
                    <td className="sl" style={{ left: 0, width: 36, minWidth: 36 }} onClick={(e) => e.stopPropagation()}>
                      <Checkbox
                        checked={isSel}
                        onCheckedChange={(v) => setSelectedIds((s) => (v ? [...s, c.id] : s.filter((x) => x !== c.id)))}
                      />
                    </td>
                    ...
                    {scope === "user" && (
                      <td title={c.principals.join(", ")}>
                        {c.principals[0] || "--"}
                        {c.principals.length > 1 && <Badge variant="outline" className="ml-1">+{c.principals.length - 1}</Badge>}
                      </td>
                    )}
                    ...
                    <td>
                      <Badge variant="outline" className={cn(c.status === "Active" && "bg-risk-green/15 text-risk-green border-risk-green/30", c.status === "Expired" && "bg-risk-red/15 text-risk-red border-risk-red/30", c.status === "Revoked" && "bg-muted text-muted-foreground border-border")}>
                        {c.status}
                      </Badge>
                    </td>
                    <td>
                      <Badge variant="outline" title={c.endpoints.join(", ")}>{c.endpoints.length}</Badge>
                    </td>
                    <td>
                      <Badge variant="outline" className={cn(c.complianceStatus === "Compliant" ? "bg-risk-green/15 text-risk-green border-risk-green/30" : "bg-risk-red/15 text-risk-red border-risk-red/30")}>
                        {c.complianceStatus}
                      </Badge>
                    </td>
                  </tr>
